    ...
    return (
      <div className="flex flex-col h-full">
        ...
        <div className="flex flex-col items-center justify-center flex-1 text-muted-foreground gap-2 px-6 text-center">
          ...
          <span className="text-xs">Add nodes to canvas to filter</span>
          <span className="text-[10px] opacity-60">Filters will appear when elements are on the canvas</span>
        </div>
      </div>
    );
  }
    ...
    <div className="flex flex-col h-full">
      ...
      <ScrollArea className="flex-1">
        <div className="px-1 py-2 space-y-1">
          {labels.length > 0 && (
            <>
              <SectionHeader>Node Labels</SectionHeader>
              {labels.map((label) => (
                <LabelRow
                  key={label}
                  label={label}
